...
export default function CreateBoard({ onCreate }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-2 p-4 bg-white rounded shadow">
      ...
      />
      <select value={privacy} onChange={(e) => setPrivacy(e.target.value as Privacy)} className="border p-2 rounded">
        <option value="workspace">Workspace</option>
        <option value="private">Private</option>
        <option value="public">Public</option>
      </select>
      <button type="submit" className="bg-blue-500 text-white p-2 rounded">Create Board</button>
    </form>
  );
}
