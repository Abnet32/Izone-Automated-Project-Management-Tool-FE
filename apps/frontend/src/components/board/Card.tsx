...
interface CardProps {
  ...
  onLabelToggle?: (cardId: string, label: string) => void;
}

export default function Card({ card, onCardClick, onLabelToggle }: CardProps) {
        ...
        <div className="flex flex-wrap gap-1 mb-2">
          {card.labels.map(label => (
            <span
              ...
              style={{ backgroundColor: LABEL_COLORS[label as keyof typeof LABEL_COLORS] }}
              onClick={(e) => {
                e.stopPropagation();
                onLabelToggle?.(card.id, label);
              }}
            >
              {LABEL_NAMES[label as keyof typeof LABEL_NAMES]}
            </span>
          ))}
        </div>
      )}
